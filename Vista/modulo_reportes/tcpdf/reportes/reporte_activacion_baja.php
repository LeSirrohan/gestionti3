...
//		$this->Cell(0, 20, 'HOJA N/TOTAL DE HOJAS '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
	...
	public function CuerpoTablaHtml($reg,$fecha_inicio,$fecha_fin,$fecha_registro)
	{
		...
		$cabecera = array('Decreto/Resuelto', 'Fecha' , 'Posición','Planilla','*T/I','Cédula Actual','Fecha de Cese','Nombres y Apellidos','Motivación de la Eliminación de la Baja','Observación');
	    ...
	    while($fila=fetch_array($res1))
	    {
			...
			$this->Cell($w[$ixx++], $altura, $fila['fecha_cese'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
			...
			$this->Cell($w[$ixx++], $altura, $fila['motivo_eliminacion_baja'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
...
$pdf->SetTitle('Activación de Baja');
$pdf->SetSubject('ReporteActivación de Baja');
