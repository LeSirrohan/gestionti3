...
//		$this->Cell(0, 20, 'HOJA N/TOTAL DE HOJAS '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
	...
	public function CuerpoTablaHtml($reg,$fecha_inicio,$fecha_fin,$fecha_registro)
	{
		...
		$cabecera = array('Decreto/Resuelto', 'Fecha' , 'Posición','Planilla','*T/I','Cédula','Nueva Posición','Nueva Planilla','Nombres y Apellidos','Sueldo','T/S','Sobresueldo','Observación');
		...
		$w = array(25, 15,13,13,13,15,20,20,30,15,15,20);
		...
		for($i = 0; $i < $num_headers; ++$i) {
			$this->Cell($w[$i], 15, $cabecera[$i],  1, 0, 'C', 1, '', 1);
	    ...
	    while($fila=fetch_array($res1))
	    {
			...
			$this->Cell($w[$ixx++], $altura, $fila['nueva_posicion'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
			$this->Cell($w[$ixx++], $altura, $fila['nueva_planilla'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
...
$pdf->SetSubject('Reporte Traslados');
