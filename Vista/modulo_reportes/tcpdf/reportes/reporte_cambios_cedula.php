...
//		$this->Cell(0, 20, 'HOJA N/TOTAL DE HOJAS '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M');
	...
	public function CuerpoTablaHtml($reg,$fecha_inicio,$fecha_fin,$fecha_registro)
	{
		...
		$sql1              = " 
		SELECT DISTINCT *
			FROM mov_contraloria as A, mov_inclusiones AS B WHERE A.id_mov_contraloria = B.id_mov_contraloria
	    ...
	    while($fila=fetch_array($res1))
	    {
			...
			$this->Cell($w[$ixx++], $altura, $fila['c002'], $border,$ln,$align,$fill,$link,$stretch,$ignore_min_height,$calign,$valign);
